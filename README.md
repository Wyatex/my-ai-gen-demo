...
# [🚀 音视频详细信息检测器 (FFprobe.wasm)](./media-info.html)

基于 **FFmpeg.wasm (v0.12.7)** 构建的纯前端多媒体信息深度检测工具，类似 `ffprobe` 命令行工具的浏览器本地实现。

## ✨ 核心亮点

- 🎨 **精美现代设计 & 深浅色无缝切换**：采用 Tailwind CSS 设计，完美适配明亮/暗黑模式，色彩过渡流畅，支持系统主题跟随与用户偏好持久化。
- 🔒 **纯前端 0 字节上传**：利用 WebAssembly 在本地读取音视频文件，完全保护多媒体隐私数据。
- 🔍 **类似 ffprobe 的深度解析**：
  - **容器与文件级别**：格式封装（MP4, MKV, WebM, MOV, AVI, MP3, FLAC, WAV, AAC 等）、完整时长、总体码率、起始时间、存储体积预估。
  - **视频流**：编解码器（H.264/AVC, H.265/HEVC, VP9, AV1, ProRes...）、配置级别（Profile）、分辨率规格（4K/2K/1080p/720p）、显示宽高比 (DAR/SAR)、帧率 (FPS/tbr/tbn)、像素格式 (yuv420p 等)、色彩空间及色彩范围。
  - **音频流**：编码格式（AAC, MP3, FLAC, Opus, PCM 等）、采样率（44.1kHz, 48kHz 等）、声道数与声道布局（Mono, Stereo, 5.1 等）、采样位深（fltp, s16p 等）、音频码率。
  - **字幕与章节**：内嵌字幕流识别、章节标记（起止时间、标题）提取。
  - **元数据标签**：完整提取 ID3/QuickTime/Vorbis 标签（歌名、歌手、专辑、年份、编码器等），支持实时关键字搜索与一键复制。
- 🎬 **原生播放与动态可视化**：
  - **视频预览**：支持进度拖拽，提供“📸 截取当前帧画面”并支持一键下载超清 PNG 原图。
  - **音频预览**：集成 Web Audio API 动态频谱分析器，播放时自动呈现绚丽的音频波形柱状动效。
- 📦 **导出与兼容**：
  - 提供与 `ffprobe -print_format json` 兼容的标准 JSON 结构查看、一键复制及 `.json` 文件导出。
  - 支持一键复制格式化文本诊断简报。
  - 完整呈现底层 FFmpeg 原始终端控制台日志，支持日志关键字过滤。
- ✨ **开箱即用内置测试样本**：内置纯 JS 实时合成的双声道 WAV 音频样本与 Canvas 30fps 动画 WebM 视频样本，无需查找本地文件即可一键体验！
